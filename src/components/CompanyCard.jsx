...
export default function CompanyCard({ slug, name, tier, count }) {
  return (
    <Link
      to={`/company/${slug}`}
      className="group block bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-indigo-500 transition-colors min-w-[160px]"
    >
      ...
      <p className="text-sm text-gray-400 mt-1">{count} questions</p>
    </Link>
  );
}
